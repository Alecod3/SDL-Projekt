    ...
    while (inMenu)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            SDL_SetRenderDrawColor(renderer,
                                   (i == selected) ? 180 : 90, 0, 0, 255);
            SDL_RenderFillRect(renderer, &buttons[i]);
            SDL_Surface *surf = TTF_RenderText_Blended(font, labels[i], (SDL_Color){255, 255, 255});
            SDL_Texture *tex = SDL_CreateTextureFromSurface(renderer, surf);
            SDL_Rect tr = {
                buttons[i].x + (buttons[i].w - surf->w) / 2,
                buttons[i].y + (buttons[i].h - surf->h) / 2,
                surf->w, surf->h};
...
int main(int argc, char *argv[])
{
    if (SDLNet_Init() < 0)
    {
        SDL_Log("SDL_net init error: %s", SDLNet_GetError());
        return 1;
    }
    ...
    while (running)
    {
        ...
        network_recv_state();
        // Rita remote-player om vi är i multiplayer
